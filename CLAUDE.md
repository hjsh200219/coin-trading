...
### Git Push 규칙
- **중요**: `git push`는 사용자가 **명시적으로 "git push" 또는 "push"를 요청했을 때만** 실행
- 자동으로 push하지 않음
- 커밋 완료 후 사용자에게 push 여부를 알리고 대기

### Git Commit 자동화
사용자가 변경사항을 커밋하려고 할 때:
1. **커밋 메시지 작성**:
   - 변경사항을 분석하여 명확하고 구조화된 **한글** 커밋 메시지 생성
   - 형식: `기능:`, `수정:`, `리팩토링:`, `문서:` 등의 한글 접두사 사용
   - 주요 변경사항을 한글로 bullet point 정리

2. **커밋 실행**:
   - `git add .` - 변경사항 스테이징
   - `git commit -m "한글 커밋 메시지"` - 커밋 실행
   - **push는 하지 않음** - 사용자가 명시적으로 요청할 때까지 대기
