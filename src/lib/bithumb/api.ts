 ...
 * 공통 fetch 로직
 */
async function fetchBithumbAPI(url: string): Promise<BithumbTickerResponse> {
  try {
...
export async function getAllTickers(): Promise<BithumbTickerResponse> {
  return fetchBithumbAPI(`${BITHUMB_API_BASE_URL}/ticker/ALL_KRW`)
}
...
export async function getTicker(symbol: string): Promise<BithumbTickerResponse> {
  return fetchBithumbAPI(`${BITHUMB_API_BASE_URL}/ticker/${symbol}_KRW`)
}
